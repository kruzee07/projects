...
# The Original iteration file for the EDA Project


# P.S. If you are worried about the length of the code - don't most of it is whitespace for readability.
########################################################## LOADING ##########################################################
...
def calculate_emissions_profiles(file_path):
    ...
    # Prompt user for grid cell center coordinates
    center_lon = float(input("Enter the longitude of the center of the grid cell: "))
    center_lat = float(input("Enter the latitude of the center of the grid cell: "))

    # Prompt user for the area size in km
    cell_size_km = float(input("Enter the grid cell size in km: "))
    km_per_deg_lat = 111.0  # Approximate conversion factor for latitude to kilometers
    km_per_deg_lon = 111.0 * math.cos(math.radians(center_lat))  # Adjust based on latitude

    cell_size_deg_lat = cell_size_km / km_per_deg_lat
    cell_size_deg_lon = cell_size_km / km_per_deg_lon

    # Calculate the boundaries of the grid cell
    min_lat = center_lat - (cell_size_deg_lat / 2)
    max_lat = center_lat + (cell_size_deg_lat / 2)
    min_lon = center_lon - (cell_size_deg_lon / 2)
    max_lon = center_lon + (cell_size_deg_lon / 2)

    # Create the grid cell as a GeoDataFrame
    grid_cell = box(min_lon, min_lat, max_lon, max_lat)
    grid_cell_gdf = gpd.GeoDataFrame([grid_cell], columns=['geometry'])
    ...
    for year in years_of_interest:
        ...
        # Perform spatial join to find emissions sources within the grid cell
        joined_cell = gpd.sjoin(yearly_data, grid_cell_gdf, how='inner', predicate='within')

        if not joined_cell.empty:
            # Aggregate emissions for the grid cell by gas type
            for idx, row in joined_cell.iterrows():
                location_key = f"Location: ({row.lon}, {row.lat})"
                if location_key not in emissions_profiles[year]:
                    emissions_profiles[year][location_key] = {}
                gas = row['gas']
                quantity = row['emissions_quantity']
                if gas not in emissions_profiles[year][location_key]:
                    emissions_profiles[year][location_key][gas] = 0
                emissions_profiles[year][location_key][gas] += quantity

    # Output the grid center
    print(f"\nGrid Center: ({center_lon}, {center_lat})")

    # Output the emissions profiles for each year
    for year, locations in emissions_profiles.items():
        print(f"\nEmissions Profiles for regions in the {cell_size_km}km grid region for year {year}:")
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            file_path = input("Enter the path to the emissions data CSV file: ").strip()
            calculate_emissions_profiles(file_path)
